...
%contrasts = {'V-A', 'A-P'};
contrasts = {'f-vP', 'f-aP', 'f-tP','aA-aP', 'vA-vP', 'tA-tP'};
...
for cc = 1:N_contrasts
    ...
    for hh = 1:length(hemis)
        ...
        % Run tfce with PALM
        palm('-i', new_input, '-d', [res_path 'design19.mat'], '-t' ,[res_path 'design.con'], '-s',...
            [fsdir hemi '.pial'], '-n', tfce_iters, '-ise', '-T', '-tfce2D', '-logp', '-o', outdir)

    end
end

%% PALM is outputting in nifti-2 format for some reason, so use __.ipynb to convert back into nifti-1 so matlab/freesurfer can use the files
