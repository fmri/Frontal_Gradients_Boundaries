...
ROI_dir = '/projectnb/somerslab/tom/projects/Frontal_Gradients_Boundaries/data/permutation_analysis_labels_etc/';
...
%COM_vert_inds = nan(N_ROIs, N_contrasts, N_hemis, N_iterations); % storage for vertex index closest to COM
...
for cc = 1:N_contrasts
    ...
    for rr = 1:N_ROIs
        ...
        for hh = 1:N_hemis
            ...
            % Load patch file
            patch_path = [ROI_dir hemi '.' keyword '_' ROI_name '_grand_probabilistic_thresh5_flat.patch'];
            patch = read_patch(patch_path);

            % Load label file
            label_path = [ROI_dir hemi '.' keyword '_' ROI_name '_grand_probabilistic.label'];
...
for hh = 1:N_hemis
    ...
    for cc = 1:N_contrasts
        ...
        % Load probabilistic data for full hemisphere
        prob_data_path = [ROI_dir hemi '_original_' contrast '_' keyword '.nii'];
        prob_data = MRIread(prob_data_path);
...
for hh = 1:N_hemis
    ...
    for rr = 1:N_ROIs
        ...
        orig_coord_diffs{counter,1} = hemis(hh);
        orig_coord_diffs{counter,2} = {ROI_name};
        orig_coord_diffs{counter,3} = squeeze(COM_WM-COM_SMC)';
        orig_coord_diffs{counter,4} = pdist([COM_WM'; COM_SMC']);
    end
end
...
for hh = 1:N_hemis
    ...
    for rr = 1:N_ROIs
        ...
        disp([hemi ' ' ROI_name ' permutation p-value=' num2str(sum(orig_data<perm_data)/(N_iterations+1))]);
    end
end
