...
for hh = 1:num_hemis
    ...
    switch axis_method % 2 possible methods for finding axis of greatest change
        case 'regression'
            ...
            Tq = griddata(ROIs{hh}.x, ROIs{hh}.y, group_data_diff{hh}, xq, yq, 'linear'); % interpolate to 100x100 grid
            figure;
            surf(xq, yq, Tq); % plot surface
            hold on;
...
for hh = 1:num_hemis
    ...
    for ss = 1:N_subjs
        ...
        %% Find best separation boundary
        figure; 
        scatter(xs, ys, [], Ts);
        clim_set = max(abs(prctile(Ts, [10,90]))); 
        xlabel('x'); ylabel('y'); cb = colorbar; colormap(redbluedark); clim([-clim_set, clim_set]); ylabel(cb, 'T-stat Difference', 'rotation', 270);

        % Define grid resolution
        gridSize = 0.05;  % adjust based on point density
        
        [xq, yq] = meshgrid( ...
            min(xs):gridSize:max(xs), ...
            min(ys):gridSize:max(ys));
        
        % Interpolate scattered data
        tic
        Tq_orig = griddata(xs, ys, Ts, xq, yq, 'natural');
        toc
        
        % Supress local minima and maxima
        Tq_nonans = Tq_orig;
        nan_inds = isnan(Tq_orig);
        Tq_nonans(isnan(Tq_orig)) = 0;
        suppress_thresh = prctile(abs(Tq_orig(:)), 90);
        Tmin = imhmin(Tq_nonans, suppress_thresh);
        Tminmax = imhmax(Tmin, suppress_thresh); % doesn't matter if min or max is first
        Tminmax(nan_inds) = nan;

        % Zero crossing detection
        Tq_pad = padarray(Tminmax, [1 1], 'replicate');
        E  = false(size(Tminmax));
        ...
        for dy = -1:1
            for dx = -1:1
                ...
                N = Tq_pad(2+dy:end-1+dy, 2+dx:end-1+dx);
                E = E | (Tminmax .* N < 0);
            end
        end
        ...
        imagesc(Tq_orig);
        colorbar;
        clim_set = max(abs(prctile(Tq_orig(:), [10,90]))); 
        colormap(redbluedark); 
        clim([-clim_set, clim_set]);
        axis tight xy;

        subplot(1,3,2);
        imagesc(Tminmax);
        colorbar;
        clim_set_minmax = max(abs(prctile(Tminmax(:), [10,90]))); 
        colormap(redbluedark); 
        clim([-clim_set_minmax, clim_set_minmax]);
        axis tight xy;

        subplot(1,3,3);
        imagesc(Tq_orig); hold on;
        contour(E, [0.5 0.5], 'g')
        colorbar;
        colormap(redbluedark); 
        clim([-clim_set, clim_set]);
        ...
        % Things I've tried already:
            % kmeans with 2 clusters. Just gives linear boundary. Not sufficient
            % Fit gaussian process to data, estimate surface, find largest
                % gradients on surface. The problem is this does not give a
                % full boundary across the whole ROI, usually just small
                % hyperlocal gradients. 
            % SVM with labels being > and < 0 T-stat and either nonlinear
            % rbf kernal or polynomial kernal. polynomial 3 kernal working ok maybe
            % Watershed on smoothed image
