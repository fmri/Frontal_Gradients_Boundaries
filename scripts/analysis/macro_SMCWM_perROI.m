...
contrasts = {'vP-f', 'vA-vP', 'aP-f', 'aA-aP', 'aPvP-f', 'vAaA-vPaP'}; %P = passive/sensorimotor control, A = active/WM, v = visual, a = auditory, f = fixation
...
PSCs = nan(N, N_ROIs, 2, N_contrasts); % percent signal change storage
coordinates = nan(N, N_ROIs, 2, N_contrasts); % store mean coordinates of ROI
...
for ss = 1:N
    ...
    for hh = 1:2
        ...
        for cc = 1:N_contrasts
            ...
            if strcmp(contrast, 'aP-f') | strcmp(contrast, 'vP-f') % these contrasts' are backwards, so change
            ...
            data = MRIread([data_dir subjCode '/localizer/localizer_contrasts_' hemi '/' contrast '/cespct.nii.gz']); % extract PSC files
            ...
            for rr = 1:N_ROIs
                ...
                if isfile(label_fpath) % if file doesn't exist, leave PSC as nan
                    ...
                    coordinates(ss,rr,hh,cc) = mean(ROI_label{:,3}); % get mean coords
...
%% To look at relationship between coords and WM/SMC ratio, analyze with LME 
% create LME tables
