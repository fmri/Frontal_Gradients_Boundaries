...
conds_perblock_str = '16trials'; % blocktrials or 16trials
save_str = 'WM_SMC';
conds_perblock = 16; %1;
conditions = [1:96]; % all visual WM, auditory WM, visual SMC, and auditory SMC
condition_labels = [ones(conds_perblock*2,1); zeros(conds_perblock,1); ones(conds_perblock*2,1); zeros(conds_perblock,1)];
...
if any(ismember(conditions, 145:160)) && any(ismember(subjCodes, {'MM', 'PP'}))
...
runlist = nan((N_subjs * N_trials_persubj)+(trials_mod*N_trials_perrun), 1); 
