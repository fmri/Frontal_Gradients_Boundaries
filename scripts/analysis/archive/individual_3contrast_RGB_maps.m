...
%%% that will display all contrasts' information by vertex/voxel for each
%%% individual subject
...
group_mask = true; % use group-level TFCE-derived significance mask for all instead of individually derived p<.05 masks
...
%subjCodes = subjCodes(15:end); %%%%%%%%%%%%%%%
N_subjs = length(subjCodes);

RGB_data = nan(fs_num, N_subjs, 3, 2); % vertices x subjs x contrasts x hemispheres
...
% Load group TFCE significance mask if needed
if group_mask
    tfce_path = '/projectnb/somerslab/tom/projects/Frontal_Gradients_Boundaries/data/group_contrast_TFCE/';
    for hh = 1:2
        contrast1_tfce_p = MRIread([tfce_path contrasts{1} '/' hemis{hh} '/_tfce_tstat_fwep_nifti1.nii']);
        contrast1_tfce_sig = contrast1_tfce_p.vol > 1.3;
        contrast2_tfce_p = MRIread([tfce_path contrasts{2} '/' hemis{hh} '/_tfce_tstat_fwep_nifti1.nii']);
        contrast2_tfce_sig = contrast2_tfce_p.vol > 1.3;
        contrast3_tfce_p = MRIread([tfce_path contrasts{3} '/' hemis{hh} '/_tfce_tstat_fwep_nifti1.nii']);
        contrast3_tfce_sig = contrast3_tfce_p.vol > 1.3;
        group_union_sig{hh} = contrast1_tfce_sig | contrast2_tfce_sig | contrast3_tfce_sig;
    end
end
...
for ss = 1:N_subjs
    ...
    for hh = 1:2
        ...
        if group_mask
            union_sig = group_union_sig{hh};
        else
            % Get pvals for each vertex in each contrast
            contrast_data1 = MRIread([contrast_path subjCode '/localizer/localizer_contrasts_' hemi '/' contrasts{1} '/sig.nii.gz']);
            if contains(contrasts{1}, 'f-') % switch contrast if it's backward
                contrast_data1.vol = -contrast_data1.vol;
            end
            contrast1_sig = contrast_data1.vol > 1.3;
    
            contrast_data2 = MRIread([contrast_path subjCode '/localizer/localizer_contrasts_' hemi '/' contrasts{2} '/sig.nii.gz']);
            if contains(contrasts{2}, 'f-') % switch contrast if it's backward
                contrast_data2.vol = -contrast_data2.vol;
            end
            contrast2_sig = contrast_data2.vol > 1.3;
    
            contrast_data3 = MRIread([contrast_path subjCode '/localizer/localizer_contrasts_' hemi '/' contrasts{3} '/sig.nii.gz']);
            if contains(contrasts{3}, 'f-') % switch contrast if it's backward
                contrast_data3.vol = -contrast_data3.vol;
            end
            contrast3_sig = contrast_data3.vol > 1.3;
            
            % Get union of all sig vertices
            union_sig = contrast1_sig | contrast2_sig | contrast3_sig;
        end
        ...
        for cc = 1:3
            ...
            % Get tstat data
            stat_path = [contrast_path subjCode '/localizer/localizer_contrasts_0sm_' hemi '/' contrasts{cc} '/t.nii.gz'];
            ...
            for vv = 1:length(ptiles)-1
                ...
                RGB_data(mask,ss,cc,hh) = RGB_vals(vv);
            end
        end
        % figure;
        % subplot(1,3,1);
        ...
        % subplot(1,3,3);
        % histogram(RGB_data(:,ss,1,hh), 'NumBins',50); hold on;
        % histogram(RGB_data(:,ss,2,hh), 'NumBins',50);
        % histogram(RGB_data(:,ss,3,hh), 'NumBins',50);
...
lh_reorder = RGB_data(frontal_verts_lh.Var1+1,:,:,1);
rh_reorder = RGB_data(frontal_verts_rh.Var1+1,:,:,2);
...
for ss = 1:N_subjs
    ...
    for hh=1:2
        ...
        write_annotation([annot_outpath hemi '.'  subjCode '_VisAudWM_3contrast_RGB_' latmed '_groupmask_0sm.annot'], verts_ref, annot_labels, annot_ctable);
        disp(['Finished ' subjCode ' ' hemi])
    end
end
