...
keyword= 'supramodal'; % used for naming nii files
...
other_outdir = '/projectnb/somerslab/tom/projects/Frontal_Gradients_Boundaries/data/permutation_analysis_labels_etc/';
...
tstat_data = nan(N_vertices, N_hemis, N_contrasts, N);
    ...
    for nn = 1:N
        ...
        for cc = 1:N_contrasts
            ...
            for ss = 1:2
                ...
                data_temp(:,ss) = data.vol>p_thresh;
            end
            tstat_data(:,hh,cc,nn) = data_temp(:,1) & data_temp(:,2);
        end
    end
end
...
for ii = 1:iterations
    ...
    subj_data1 = squeeze(tstat_data(:,:,1,:)); % select all WM data
    subj_data1(:,:,current_perm==1) = squeeze(tstat_data(:,:,2,current_perm==1)); % replace WM with SMC data where perm is 1

    subj_data2 = squeeze(tstat_data(:,:,2,:)); % select all SMC data
    subj_data2(:,:,current_perm==1) = squeeze(tstat_data(:,:,1,current_perm==1)); % replace SMC with WM data where perm is 1
...
MRIwrite(data, [other_outdir, 'lh_grand_probmap_' keyword '.nii']);
data.vol = grand_probmap(:,2)';
MRIwrite(data, [other_outdir, 'rh_grand_probmap_' keyword '.nii']);
grand_probmap_thresh = iterations*N_contrasts*N * 0.1;

%% Create probabilistic map for unpermuted data

WM_data = squeeze(tstat_data(:,:,1,:));
SMC_data = squeeze(tstat_data(:,:,2,:));
...
MRIwrite(data, [other_outdir, 'lh_original_WM_' keyword '.nii']);
data.vol = SMC_probmap(:,1)';
MRIwrite(data, [other_outdir, 'lh_original_SMC_' keyword '.nii']);
...
MRIwrite(data, [other_outdir, 'rh_original_WM_' keyword '.nii']);
data.vol = SMC_probmap(:,2)';
MRIwrite(data, [other_outdir, 'rh_original_SMC_' keyword '.nii']);
